...
namespace Services
{
    public class CustomerService : ICustomerService
    {
        ...
        public  PaginationResponse<CustomerEntity> GetCustomers(QueryParameters queryParameters)
        {
            ...
            return new PaginationResponse<CustomerEntity>(result, result.Count, queryParameters.Pagination.PageNumber, queryParameters.Pagination.PageSize);
        }
        protected static IQueryable<CustomerEntity> TextSearchQuery(QueryParameters queryParameters, IQueryable<CustomerEntity> query)
        {
            if (!String.IsNullOrEmpty(queryParameters.SearchTerm) && !String.IsNullOrWhiteSpace(queryParameters.SearchTerm))
            {
                switch (queryParameters.SearchColumn)
                {
                    ...
                    case nameof(CustomerEntity.FiscalIdentification):
                        query = query.Where(a => a.FiscalIdentification.ToLower().Contains(queryParameters.SearchTerm.ToLower()));
                        break;
                    default:
        ...
        protected static IQueryable<CustomerEntity> DateSearchQuery(QueryParameters queryParameters, IQueryable<CustomerEntity> query)
        {
            if (queryParameters.StartDate.HasValue && queryParameters.EndDate.HasValue)
            {
                switch (queryParameters.DateField)
                {
                    ...
                    case nameof(CustomerEntity.LastInteraction):
                        query = query.Where(a => a.LastInteraction >= queryParameters.StartDate && a.LastInteraction <= queryParameters.EndDate);
                        break;
                    default:
        ...
        protected static IQueryable<CustomerEntity> Pagination(QueryParameters queryParameters, IQueryable<CustomerEntity> query)
        {
        ...
        protected static IQueryable<CustomerEntity> SortQuery(QueryParameters queryParameters, IQueryable<CustomerEntity> query)
        {
            if (!String.IsNullOrEmpty(queryParameters.Sort.SortBy) && !String.IsNullOrWhiteSpace(queryParameters.Sort.SortBy))
            {
                switch (queryParameters.Sort.SortBy)
                {
                    ...
                    case nameof(CustomerEntity.UpdatedAt):
                        ...
                        break;
                    case nameof(CustomerEntity.LastInteraction):
                        query = queryParameters.Sort.Ascending ? query.OrderBy(a => a.LastInteraction) : query.OrderByDescending(a => a.LastInteraction);
                        break;
                    case nameof(CustomerEntity.FiscalIdentification):
                        query = queryParameters.Sort.Ascending ? query.OrderBy(a => a.FiscalIdentification) : query.OrderByDescending(a => a.FiscalIdentification);
                        break;
                    default:
